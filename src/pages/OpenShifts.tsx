...
export default function OpenShifts() {
      ...
      fetch(`http://localhost:4000/employments/account/${user.id}`)
        ...
        .then((employmentData) => {
          if (employmentData.employmentType) {
            const isOpen = employmentData.employmentType.toLowerCase() === "open";
            setIsOpenEmployee(isOpen);
          } else {
  ...
  useEffect(() => {
    ...
    if (userId !== null && isOpenEmployee) {
      fetchAttendance();
    }
  }, [userId, isOpenEmployee]);
  ...
  if (!isOpenEmployee) {
    return (
      <div className="p-6 rounded-lg shadow-md border border-gray-100 dark:border-gray-800 text-sm text-gray-700 dark:text-gray-200">
        <p className="text-center text-red-500">This page is only accessible to open employees.</p>
      </div>
    );
  }
  ...
  return (
    <>
      <PageBreadcrumb pageTitle="Home / Hours / Open Logs" />
      <div className="p-6 rounded-lg shadow-md border border-gray-100 dark:border-gray-800 text-sm text-gray-700 dark:text-gray-200">
